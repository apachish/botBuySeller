...
class DeleteMessage extends Command
{
    ...
    public function handle()
    {
        ...
        if ($bot_user) {
            $telegram_services = new TelegramServices($bot_user->tokrn);
            $message = MessageAdmin::where("message_id", $message_id);
            if ($message)
                $telegram_services->deleteMessage($message->user_id, $message_id);
        }
